...
int main()
{
    ...
    printf("\t\t\t\t\tER. C PROGRAMMING LOGIN PAGE\n\n\n");
     space(3);printf("Introduction: Welcome to Er. C programming Login Page.\n");
      space(3);printf("Your system default Username and Password is \"admin\"\n\n");
     if(azq>=1)
    {
        printf("\t\tWRONG LOGIN\n\n");
    }
     space(3);printf("Enter your username: ");
    scanf("%s", username);
    printf("\n");
    int i = 0;
    char ch;
     space(3);printf("Enter your password: ");
    while ((ch = _getch()) != '\r')
    {
        if (ch == '\b')
        {
...
void afterLogin()
{
    ...
    space(10);printf("Word By Devs:\t Welcome to Er.C solution App.\n\t\t\t This app is made to help\n\t\t\t student who want to learn c\n\t\t\t programming easily.\n\t\t\t we provide you the full\n\t\t\t solution so it helps you in\n\t\t\t our exam.\n\n");
     ...
     space(10);printf("Developer:\tArpan,Sugam,Sujan\n");

    printf("-----------------------------------------------------------------\n\n");

     space(10);printf("1. All past year question (2069-2080)\n");

     space(10);printf("2. Repeated past year question (2069-2080)\n");

     space(10);printf("3. Palindrome, Fibonacci, Armstrong questions (2069-2080)\n");

     space(10);printf("4. \"SOURCE CODE\" of past year question (Important only)\n");

     space(10);printf("5. Add your question with solution\n");

     space(10);printf("6. Your saved questions solution\n");

    wrongInput:

     space(10);printf("Enter the category number to go to that section: ");
...
//function for source and rep only quest
   repSourceDecision();
...

        {
            ...
            int n, sum = 0, temp3, remainder, digits = 0;
    start:
...
void YourSavedAnswer()
{
    FILE*fpabc;
    FILE*fpdef;
    fpabc=fopen("questiononlyfile.txt","r");
    fpdef=fopen("answeronlyfile.txt","r");
    struct questiononly qQ[30];
    struct answeronly qA[30];
    int n=0;
    if(fpabc=NULL)
   ...
   while (fread(&qQ[n], sizeof(struct questiononly), 1, fpabc) == 1)
    ...
    fclose(fpdef);
     ...
     //user choice selection using seperate function
    repSourceDecision();
